...
experiment = 'cosmo_primary_256_200k_batchSize%i_flipLabel%0.3f_learningRate%0.5f_'\
             'nd%i_ng%i_gfdim%i_dfdim%i_zdim%i_nodes%i_rank%i'%(batch_size, flip_labels, learning_rate, nd_layers,\
                                                 ng_layers, gf_dim, df_dim, z_dim, numnodes, nodeid)

command = 'python -u -m models.main --dataset cosmo --datafile %s '\
          '--learning_rate %d --output_size %i --flip_labels %f --experiment %s '\
          '--epoch %i --batch_size %i --z_dim %i '\
          '--nd_layers %i --ng_layers %i --gf_dim %i --df_dim %i --save_every_step %s '\
          '--data_format %s --transpose_matmul_b %s --verbose %s --num_inter_threads %i --num_intra_threads %i'%(datafile, learning_rate, output_size, flip_labels, experiment,\
