...
#argument parsing, makes things simpler
import argparse

#set up parser
parser = argparse.ArgumentParser(description='Run LIKWID on cosmogan.')
parser.add_argument('--group', metavar='-g', type=str, nargs=1, help='likwid counter group.')
...
experiment = 'cosmo_primary_256_50k_batchSize%i_flipLabel%0.3f_'\
             'nd%i_ng%i_gfdim%i_dfdim%i_zdim%i_likwid%s_nodes%i_rank%i'%(batch_size, flip_labels, nd_layers,\
                                                 ng_layers, gf_dim, df_dim, z_dim, group, numnodes, nodeid)
