...
command = 'python -m models.main --model cramer_dcgan --dataset cosmo --datafile %s '\
          '--output_size %i --learning_rate %f --flip_labels %f --experiment %s '\
          '--epoch %i --batch_size %i --z_dim %i '\
          '--nd_layers %i --ng_layers %i --gf_dim %i --df_dim %i --save_every_step %s '\
          '--data_format %s --transpose_matmul_b %s --verbose %s --num_inter_threads %i --num_intra_threads %i'%(datafile, output_size, learning_rate, flip_labels, experiment,\
