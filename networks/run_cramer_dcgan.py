...
experiment = 'cramer_dcgan_cosmo_primary_256_200k_batchSize%i_learningRate_%0.6f_nUp%i_gradientLambda%0.3f_flipLabel%0.3f_'\
             'nd%i_ng%i_gfdim%i_dfdim%i_zdim%i_nodes%i_rank%i'%(batch_size, learning_rate, n_up, gradient_lambda, flip_labels, nd_layers,\
                                                                 ng_layers, gf_dim, df_dim, z_dim, numnodes, nodeid)

command = 'python -m models.main --model cramer_dcgan --dataset cosmo --datafile %s '\
          '--output_size %i --learning_rate %f --n_up %i --gradient_lambda %f --flip_labels %f --experiment %s '\
          '--epoch %i --batch_size %i --z_dim %i '\
          '--nd_layers %i --ng_layers %i --gf_dim %i --df_dim %i --save_every_step %s '\
          '--data_format %s --transpose_matmul_b %s --verbose %s --num_inter_threads %i --num_intra_threads %i'%(datafile, output_size, learning_rate, n_up, gradient_lambda, flip_labels, experiment,\
