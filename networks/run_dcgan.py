...
def main():
    ...
    command = 'python -u -m models.main --dataset cosmo --datapath %s --fs_type %s '\
            ...
            '--epoch %i --batch_size %i --learning_rate %f --num_updates %i --z_dim %i '\
            ...
            '--num_inter_threads %i --num_intra_threads %i'%(datapath, fs_type, output_size, c_dim, experiment,\
                                                             epoch, batch_size, learning_rate, n_up, z_dim,\
    ...
    with open(outfile, 'w') as f_out:
        proc = sp.Popen(shlex.split(command), stdout=f_out, stderr=sp.PIPE, bufsize=1, universal_newlines=True, env=my_env)
        out, err = proc.communicate()
        print(err)
    #subprocess.call(shlex.split(command), stdout=f_out, shell=True)
