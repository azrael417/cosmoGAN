...
command = 'python -m models.main --dataset cosmo --datafile %s '\
          ...
          '--data_format %s --transpose_matmul_b %s --verbose %s --num_intra_threads %i --num_inter_threads %i'%(datafile, output_size, flip_labels, experiment,\
                                                                   epoch, batch_size, z_dim,\
                                                                   nd_layers, ng_layers, gf_dim, df_dim, save_every_step,\
                                                                   data_format, transpose_matmul_b, verbose, 33, 2)
